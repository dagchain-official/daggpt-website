/**
 * AI Streaming Service - Based on Bolt.new
 * Handles Claude API streaming via backend
 */

const WORK_DIR = '/home/project';

/**
 * Get system prompt (based on Bolt.new)
 */
...
<artifact_info>
  ...
  <artifact_instructions>
    ...
    11. CRITICAL: Always provide the FULL, updated content of the artifact. This means:
      ...
      - ALWAYS show the complete, full file contents when updating files
...
/**
 * Stream text from Claude via backend API
 */
export async function streamTextFromClaude(messages, onChunk, onComplete, onError) {
  try {
    const API_URL = process.env.NODE_ENV === 'production' 
      ? '/api/generate-website'
      : 'http://localhost:3001/api/webapp/stream';
...
export function parseArtifact(text) {
  const artifactMatch = text.match(/<boltArtifact[^>]*>([\s\S]*?)<\/boltArtifact>/);
  ...
  const actions = [];
  const actionRegex = /<boltAction\s+type="([^"]*)"(?:\s+filePath="([^"]*)")?>([^]*?)<\/boltAction>/g;
